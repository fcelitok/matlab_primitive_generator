...
primID = 1;
% for loop 
% if (backward == 1) then you can go with backward loop 
for k = unique(crowns)
    ...
    for a1=1:length(angles)
        ...
        for h=1:length(y)
            if (backward == 0)
                ...
                if(angles(a1) == atan2(y(h),x(h))+pi || angles(a1)+2*pi == atan2(y(h),x(h))+pi)
                    [a1 angles(a1)];
                    plot(x(h)*resolution, y(h)*resolution, 'r*')
                    current_h = h;
                end
        ...
        if(current_h~=-1)
            for b=-min(k-1,brench_limit_position):min(k-1,brench_limit_position)
                ...
                for a=-min(k-1,brench_limit_angle):min(k-1,brench_limit_angle)
                    ...
                    if(backward == 0)
                        [clothoid_check, p, L ] = clotoide([0 0 a_start], [x(select) * resolution, y(select) * resolution, a_end], numberOfIntermediatePose)
                    else
                       [clothoid_check, p, L ] = clotoide([x(select) * resolution, y(select) * resolution, a_end], [0 0 a_start], numberOfIntermediatePose) 
                    % reverse p
                    % fix angle
                    % put forward and backward
                    % add cost for backward prim
                    end
                 
                        if (clothoid_check == true)
                            ...
                            primitives(primID).additionalActionCostMult = L;
...
%                             r_curv >= min_radius && ...
%                             angdiff(a_end,a_start)<=max_beta && ...
%                             angdiff(a_start,atan2(y(select),x(select)))>0 && ...
%                             angdiff(a_end,atan2(y(select),x(select)))<0 ...
%                             ... % && angdiff(a_end,a_start)<angdiff(a_start,atan2(y(select),x(select))) ...
%                             ) || ...
%                             (a_start>a_end && a_start > -3.14 &&...
%                             r_curv >= min_radius && ...
%                             angdiff(a_start,a_end)<=max_beta && ...
%                             angdiff(a_start,atan2(y(select),x(select)))<0 && ...
%                             angdiff(a_end,atan2(y(select),x(select)))>0 ...
%                             ... % && angdiff(a_end,a_start)>angdiff(a_start,atan2(y(select),x(select))) ...
%                             ) || ...
...
%                             ...%(a_start-atan2(y(select),x(select)))<0 && ...
%                             ...%(a_end-atan2(y(select),x(select)))>0 ...
%                             ...%) || ...
%                             (a_start>a_end && a_start > -3.14 && a_end <= -3.14 &&...
%                             r_curv >= min_radius && ...
%                             angdiff(a_end,a_start)<=max_beta && ...
...
%                             end
%                         end
                    elseif(a_start==a_end && ((a_start==atan2(y(select),x(select)) || (a_start+2*pi)==atan2(y(select),x(select)))))
%                         primitives(primID).start_pose = [0, 0, find(angles == a_start)-1];
%                         primitives(primID).end_pose = [x(select),y(select), find(angles == a_end)-1];
%                         primitives(primID).real_start_pose = [0 0 a_start];
%                         primitives(primID).real_end_pose = [x(select) * resolution, y(select) * resolution, a_end];
