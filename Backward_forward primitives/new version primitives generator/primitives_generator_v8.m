...
wheelbase = 0.65; % in meeters
steering_angle_max = 22*pi/180; % in radiants for Agilex hunter 2.0 max steering angle 22 degree 
numberOfAngle = 32; %16, 32, 48
numberOfIntermediatePose = 10;
resolution = 0.15;
map_resolution = 0.05;
max_dist = 1.5; %tested up tp 2.0m
linear_log = 'log';  %value could be 'lin' or 'log'
            ...
            for b=-min(k-1,brench_limit_position):min(k-1,brench_limit_position)
                ...
                for a=-min(k-1,brench_limit_angle):min(k-1,brench_limit_angle)
                    ...
                    [clotoid_check, p, L ] = clotoide([0 0 a_start], [x(select) * resolution, y(select) * resolution, a_end], numberOfIntermediatePose, min_radius);
                     ...
                     if(a_start==a_end && ((a_start==atan2(y(select),x(select)) || (a_start+2*pi)==atan2(y(select),x(select)))))
                        ...
                        [checker, primitives(primID).intermediate_poses, L] = ...
                            clotoide(primitives(primID).real_start_pose, primitives(primID).real_end_pose, numberOfIntermediatePose, min_radius);
            ...
            if(current_h~=-1)
                for b=-min(k-1,brench_limit_position):min(k-1,brench_limit_position)
                    ...
                    for a=-min(k-1,brench_limit_angle):min(k-1,brench_limit_angle)
                        ...
                        if(a_start==a_end && (a_start==atan2(y(select),x(select))-pi || round(a_start,3)==round(atan2(y(select),x(select))-pi,3) || (a_start-pi)==atan2(y(select),x(select)) || round((a_start-pi),3)== round(atan2(y(select),x(select)),3) ))  %solve round  
                            primitives(primID).start_pose = [0, 0, find(angles == a_start) - 1];
                            primitives(primID).end_pose = [x(select), y(select), find(angles == a_end) - 1];
                            primitives(primID).real_start_pose = [0 0 a_start];
                            primitives(primID).real_end_pose = [x(select) * resolution, y(select) * resolution, a_end];
...
primID-1
generate_file(primitives, numberOfAngle, map_resolution);
